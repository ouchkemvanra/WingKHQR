//
//  KHQRActivityViewController.swift
//  WingKHQRTest
//
...
@objcMembers final class RevampKHQRActivityViewController: UIActivityViewController{
  ...
  override func viewDidLoad() {
      ...
      for (index, item) in activityItems.enumerated() {
          if index > 0 && attributedString.length > 0 && (item is String || item is URL || item is OptionalTextActivityItemSource) {
          ...
          } else if let text = item as? OptionalTextActivityItemSource{
            attributedString.append(NSAttributedString(string: text.text, attributes: baseAttributes))
          }
      }
